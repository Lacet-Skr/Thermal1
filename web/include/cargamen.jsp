<%-- 
    Document   : cargamen
    Created on : 20/09/2019, 11:07:48 AM
    Author     : LACET
--%>

<%@page import="java.util.ArrayList"%>
<%@page import="Clase.Menus"%>
<%@page import="Control.MenuCon"%>
...
.tbl tr:first-child td:last-child{
    ...
    <body>
        <%
            ...
            ArrayList<Menus> list = MenuCon.listarMenu(nombre);
            String pag = request.getParameter("pag");
            String link = null;
            String evento = request.getParameter("evento");
            //el mismo proceso para alumnos, clases y calificaciones
             if("permiso".equals(pag)){
                //link = "/thermal/Controlador?accion=guardar&tab=usuarios12&evento=buscar&par=";
            }
...
<div class="tbl">
        <table width="100%" border="0">
            <tbody>
                ...
                <% for (Menus u : list) {%>
                <tr>
                    <td align="center"><%= u.getIdmenu()%></td>
                    <td align="center" onclick="add_row(<%= u.getIdmenu()%>, '<%= u.getMenu()%>')"><%= u.getMenu()%></td>
                </tr>
                <%}%>
            </tbody>
        </table>
