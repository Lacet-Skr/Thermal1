...
.tbl tr:first-child td:last-child{
    ...
    <body>
        <%
            ...
            //el mismo proceso para alumnos, clases y calificaciones
        ...
        <table width="100%" border="0">
            <tbody>
                ...
                <tr>
                    ...
                    <td align="center"><%= pr.getDireccionproveedor() %></td>
                    <td align="center"><%= pr.getRfcproveedor()%></td>
                    <td align="center"><%= pr.getTelefonoproveedor()%></td>
                    <td align="center"><%= pr.getContactoproveedor()%></td>
                   
                      <% if("proveedores".equals(pag)) {%>
                        "<td><a href='<%= link + pr.getIdproveedor() %>' > Ver</a></td>" <%--linea para proveedores --%>
                      <% }else if("compras".equals(pag)){%>  
                        <td align='center'><input type='button' value='Ver' class='Agregar' onclick="agrega_prov('<%= pr.getNombreproveedor() %>','<%= pr.getDireccionproveedor()%>','<%= pr.getRfcproveedor()%>','<%= pr.getTelefonoproveedor()%>','<%= pr.getContactoproveedor()%>')"></td> <%--linea para proveedores --%>
                     <% }%>
                </tr>
                <%}%>
            </tbody>
